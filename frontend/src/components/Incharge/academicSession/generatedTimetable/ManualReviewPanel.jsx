...
//
// CHANGED (this pass — skip/expanded state survives the PANEL itself being
// remounted, not just BatchGroupCard):
//   - Lifting skippedIds/expandedBatches up into ManualReviewPanel (previous
//     pass) only protects against a BatchGroupCard instance individually
//     unmounting. It does NOT help if ManualReviewPanel itself gets torn
//     down and recreated — e.g. a parent that conditionally renders it
//     (`{tab === 'review' && <ManualReviewPanel .../>}`), swaps it out when
//     switching which batch/session is being reviewed, or renders it with a
//     `key` that changes. Any React state anywhere in a subtree is gone the
//     moment that subtree unmounts, regardless of how high up it lives.
//   - Fix: skippedIds and expandedBatches are now backed by sessionStorage,
//     namespaced per `session._id` (`manualReviewPanel:<sessionId>:skipped`
//     / `:expanded`). They're read from storage on mount / whenever
//     session._id changes, and written back on every change. This makes
//     the data survive the panel unmounting and remounting entirely — the
//     component instance no longer matters, only the session id does.
//   - Still intentionally NOT persisted to the server, and sessionStorage
//     itself clears on tab close / actual page reload — that limitation
//     was already called out and is unchanged. What's fixed is state
//     getting wiped by ordinary in-app navigation between batches/tabs.
//
// CHANGED (earlier pass — skip/collapsed state no longer wiped on refetch):
//   - Root cause: BatchGroupCard kept its own `skippedIds` (Set) and
//     `collapsed` state as LOCAL useState. That state lives on the React
//     component instance, which only survives across re-renders as long as
//     the same instance stays mounted at the same key. Two things could
//     blow that away:
//       1. `batchKeyFor` joined `assignment_id.batch_ids` in WHATEVER order
//          the server returned them in. That order isn't guaranteed to be
//          stable between two fetchPendingItems() calls (e.g. the resolve
//          -> refetch that runs after every confirm), so the same logical
//          batch could get a different composite key on the next fetch.
//          BatchGroupCard is keyed by that string, so a changed key means
//          React unmounts the old instance and mounts a fresh one — local
//          state (skipped ids, collapsed/expanded) gone.
//       2. Even with a stable key, a batch whose last active item just got
//          resolved (nothing left but skipped items in a different batch
//          shape, or the group temporarily empties in a race) could drop
//          out of `groups` for a tick and remount later with fresh state.
//     Fix:
//       - `batchKeyFor` now sorts batch names before joining, so the same
//         set of batches always produces the same string regardless of
//         array order returned by the API.
//       - `skippedIds` (which items are deferred) and the expand/collapse
//         state for each batch card are lifted OUT of BatchGroupCard and
//         into ManualReviewPanel itself. The panel component is mounted
//         once per `session` and never unmounts on a resolve -> refetch
//         cycle, so this state is now immune to any per-batch remount
//         weirdness entirely — it doesn't matter if a BatchGroupCard
//         instance gets torn down and rebuilt, the source of truth lives
//         one level up and is just passed back in as props.
//       - On a successful resolve, the resolved item's id is also removed
//         from `skippedIds` (cleanup — it can never appear again since
//         it's no longer pending, so no reason to keep it in the set).
//   - This is still UI-only state (not persisted server-side), so a full
//     page reload still clears it — that part was already called out as
//     a known first-pass limitation and is unchanged here. What's fixed
//     is state getting wiped *mid-session* by ordinary resolve actions.
//
// CHANGED (earlier pass — flow + consolidation):
//   - Hard flow order is now enforced: Pick Occurrences -> Unplaced ->
//     Overflow. Previously "locked" only covered overflow/unplaced while
//     choose_occurrences was pending; unplaced and overflow were both
//     unlocked together, so an admin could resolve overflow placements
//     before an unplaced item freed/consumed a cell it might have wanted.
//     Now overflow stays hidden until BOTH choose_occurrences AND unplaced
//     are clear for that batch.
//   - One card per batch (BatchGroupCard) instead of up to three separate
//     bordered sections repeating the batch name. The card shows a small
//     step tracker (done / active / locked) and only renders the items for
//     whichever stage is currently active — killed the "everything open at
//     once" repetition, and the admin isn't re-reading the batch header for
//     every kind of item.
//   - UnplacedItemCard no longer has its own bespoke day-pill picker for
//     minor/OE slots. It now renders the same BatchTimetableGrid used for
//     overflow (mode="place"), with every non-minor/OE cell forced
//     "blocked" and the minor(G)/OE(H) columns colored by
//     /minor-oe-availability. Picks are selected directly on the real
//     grid and show up there immediately (previously the picks only lived
//     in a separate list of pills, disconnected from the timetable).
//
// CHANGED (earlier pass — highlight target course on grid):
//   - OverflowItemCard and ChooseOccurrencesItemCard now pass
//     highlightAssignmentId={item.assignment_id?._id} down to every
//     BatchTimetableGrid they render, so cells already holding a session
//     for the course currently being resolved get ringed/badged (see
//     BatchTimetableGrid's TARGET_RING). Both cards also gained a matching
//     legend swatch so the admin knows what the ring means before they hit
//     it. UnplacedItemCard intentionally does NOT get this — an "unplaced"
//     item has zero existing sessions by definition, so there's nothing to
//     highlight.
//
// CHANGED (earlier pass — collapsible batch cards, compact by default, no
// full-panel reload on every resolve):
//   - BatchGroupCard is collapsible. A chevron in the header (the whole
//     header is clickable) toggles it; collapsed is the DEFAULT state for
//     every card, so a session with many batches opens compact instead of
//     dumping every pending item on screen at once.
//   - Collapsing hides the content via CSS ("hidden" class) instead of
//     conditionally unmounting it. That matters because the item cards
//     inside (Overflow/Unplaced/ChooseOccurrences) hold their own local
//     state — expanded timetable, in-progress picks/placements — and we
//     don't want collapsing the parent card to wipe out a half-made
//     selection.
//   - The panel used to gate its ENTIRE return on `isFetching`, so every
//     resolve -> fetchPendingItems() refetch replaced the whole tree with
//     the big centered spinner, which read as a full page reload even
//     though it was just a background refresh. Now a `hasLoadedOnce` flag
//     restricts that full-page spinner to the very first load; every
//     subsequent fetch (including the ones triggered after a resolve)
//     instead shows a small, non-blocking "Refreshing…" pill in the corner
//     while the existing groups stay mounted and interactive underneath.
//
// CHANGED (earlier pass — skip/defer a stage item without breaking the
// flow):
//   - The hard choose -> unplaced -> overflow order is still enforced, but
//     an admin can now mark any individual item "Skip for now" instead of
//     being forced to resolve it before the batch moves on. A course an
//     admin doesn't want to touch right now (special nature, needs a
//     colleague's input, etc.) no longer blocks every later item in that
//     stage — or the later stages — from being worked on.
//   - Skipped items are tracked per item id and pulled OUT of the stage
//     that determines activeStep, so the flow advances as soon as
//     everything ELSE in that stage is resolved. They reappear in a
//     standing "Deferred" section at the bottom of the card — visible
//     regardless of which stage is currently active — where the admin can
//     resolve or un-skip them whenever they're ready. A "Move back to
//     queue" button un-skips.
//   - If a stage was advanced past purely because its remaining items were
//     skipped (not actually resolved), the step tracker shows that stage
//     as "Deferred" (amber) rather than "Done" (green), so the admin can
//     still see at a glance that something in that stage is outstanding.
